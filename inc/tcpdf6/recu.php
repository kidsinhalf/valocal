<?php
$id=intval($_GET['id']);
$reponse=db_select('SELECT * FROM achats WHERE id=?', array($id));
if (!empty($reponse)){
$donnees=array_map('utf8_decode',$reponse[0]);
$id_achats=$donnees['id'];
$id_personnes=$donnees['fournisseur'];
...
$pdf->SetTitle('Recu SCIC VALOCAL');
$pdf->SetSubject('Recu SCIC VALOCAL');
$pdf->SetKeywords('Recu SCIC VALOCAL');
